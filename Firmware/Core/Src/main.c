...
const osThreadAttr_t readRFIDTask_attributes = { .name = "readRFIDTask",
...
const osThreadAttr_t BluetoothRXTask_attributes = { .name = "BluetoothRXTask",
		.stack_size = 128 * 4, .priority = (osPriority_t) osPriorityNormal2, };
/* Definitions for readBatteryVolt */
osThreadId_t readBatteryVoltHandle;
const osThreadAttr_t readBatteryVolt_attributes = { .name = "readBatteryVolt",
		.stack_size = 128 * 4, .priority = (osPriority_t) osPriorityLow, };
/* Definitions for broadcastTask */
osThreadId_t broadcastTaskHandle;
const osThreadAttr_t broadcastTask_attributes = { .name = "broadcastTask",
		.stack_size = 128 * 4, .priority = (osPriority_t) osPriorityHigh, };
/* Definitions for BluetoothRX */
osSemaphoreId_t BluetoothRXHandle;
const osSemaphoreAttr_t BluetoothRX_attributes = { .name = "BluetoothRX" };
/* USER CODE BEGIN PV */
...
int main(void) {
	...
	readRFIDTaskHandle = osThreadNew(StartRFIDTask, NULL,
			&readRFIDTask_attributes);

	/* creation of BluetoothRXTask */
	BluetoothRXTaskHandle = osThreadNew(StartBluetoothTask, NULL,
			&BluetoothRXTask_attributes);

	/* creation of readBatteryVolt */
	readBatteryVoltHandle = osThreadNew(StartBatteryTask, NULL,
			&readBatteryVolt_attributes);

	/* creation of broadcastTask */
	broadcastTaskHandle = osThreadNew(StartBroadcastTask, NULL,
			&broadcastTask_attributes);
...
static void MX_GPIO_Init(void) {
	...
	GPIO_PIN_1 | GPIO_PIN_2 | NIRQ_IN_Pin | GPIO_PIN_6 | GPIO_PIN_7,
			GPIO_PIN_RESET);
	...
	HAL_GPIO_WritePin(GPIOB,
			GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_12 | GPIO_PIN_13
